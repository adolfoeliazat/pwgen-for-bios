...
function autoGetBiosPwd(serial){
    if (serial.length < 3){
        console.log('Usage: biosPwd CODE/SERIAL')
        process.exit(1)
    }
    var temp = "";
    ...
    for (var key in ret_arr) {
        if(Array.isArray(ret_arr[key]) && ret_arr[key].length == 1){
            console.log(key,":",ret_arr[key][0])
        }
        else if(Array.isArray(ret_arr[key])){
            console.log(key,":")
            for(var i=0; i<ret_arr[key].length; i++){
                console.log("    ", ret_arr[key][i])
            }
        }
        else{
            console.log(key,":",ret_arr[key])
        }
    }
}

autoGetBiosPwd(process.argv)
